...
`ascii-banner.txt` is the terminal banner: the rack-fang logo mark stacked above the
wordmark in the `pagga` figlet font. Use it for CLI splash screens, MOTDs, and READMEs
where you control the output.

Note: this is not the community-scripts LXC header. Those headers are auto-generated by
upstream CI as `figlet -f slant "$APP"` and cannot carry custom art. See the closing notes
on issue #1954.
...
Avoid `lolcat` here: its rainbow cycle drifts off the brand purple. Use the gradient above
for a brand-accurate Dracula fade.
...
The rack mark is hand-built block art (downward fangs plus three device slots) matching
`logo-mark.svg`. Edit `ascii-banner.txt` directly to adjust it.
